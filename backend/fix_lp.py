"""
Fix LP v2: Using web3 contract for proper encoding
"""
...
print(f'\n--- Approving {half_cbbtc / 1e8:.8f} cbBTC ---')
if not send_tx(cbbtc_contract.functions.approve(Web3.to_checksum_address(ROUTER), half_cbbtc)):
    print('Approve failed!')
    exit(1)

# Step 2: Swap half cbBTC -> USDC
print(f'\n--- Swapping cbBTC -> USDC ---')
deadline = int(time.time()) + 1200
routes = [(Web3.to_checksum_address(CBBTC), Web3.to_checksum_address(USDC), False, Web3.to_checksum_address(FACTORY))]
if not send_tx(router.functions.swapExactTokensForTokens(half_cbbtc, 0, routes, account.address, deadline), gas=300000):
    print('Swap failed!')
    exit(1)
...
if send_tx(router.functions.addLiquidity(
